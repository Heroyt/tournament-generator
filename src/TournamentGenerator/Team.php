...
class Team extends Base implements Exportable
{
     ...
     * @var int|string The unique identifier of the team
     */

    /** @var Game[] A list of games played by this team */
    protected array $games = [];

    /** @var array Multi-dimensional associative array of number of games together with other teams */
     ...
     * @param null|int|string $id   Unique identifier of the team
     *
     * @throws InvalidArgumentException if the provided argument id is not of type 'null' or 'string' or 'int'
     */
    public function __construct(string $name = 'team', null|int|string $id = null) {
        $this->setName($name);
    ...
    public function getGamesInfo(int|string $groupId) : array {
        return array_filter($this->getGroupResults($groupId), static fn ($k) : bool => 'group' !== $k, ARRAY_FILTER_USE_KEY);
    }

    /**
     * Gets team statistics from the given group.
     *
     * @param null|int|string $groupId Unique identifier of the group to get its results
     *
     ...
     * @throws Exception if the group with given groupId doesn't exist
     */
    public function getGroupResults(null|int|string $groupId = null) : array {
        if (isset($groupId)) {
     ...
     *
     * @param null|Team  $team  A team to get the games with
     * @param null|Group $group A group from where to get the games
     *
    ...
    public function getGameWith(?Team $team = null, ?Group $group = null) : array|int {
        ...
        if (isset($team)) {
            ...
            foreach ($this->gamesWith as $id => $games) {
                $filter = array_filter($games, static fn ($key) : bool => $key === $team->getId(), ARRAY_FILTER_USE_KEY);
                if (count($filter) > 0) {
                    $return[$id] = $filter;
                }
            }
     ...
     *
     * @param null|Group      $group   A group to get its game from
     * @param null|int|string $groupId An id of group to get its game from
     *
     * @return Game[] Games from a group or all games if both arguments are null
     */
    public function getGames(?Group $group = null, null|int|string $groupId = null) : array {
