...
class HierarchyContainerTest extends TestCase
{
    public function testHierarchyBasic() : void {
        $hierarchyContainer = new HierarchyContainer(0);

        self::assertNull($hierarchyContainer->getLevelType());
        ...
        $hierarchyContainer->insert($round1, $round2, $round3, $round4);

        self::assertEquals(Round::class, $hierarchyContainer->getLevelType());
        self::assertCount(4, $hierarchyContainer);
        self::assertCount(4, $hierarchyContainer->getTopLevel());
        ...
        self::assertCount(20, $hierarchyContainer);
        self::assertCount(16, $hierarchyContainer->getLeafIds());
        self::assertCount(4, $hierarchyContainer->getTopLevel());

        $result = $hierarchyContainer->getHierarchyLevel(Round::class);
        ...
        $result = $hierarchyContainer->getHierarchyLevel(Group::class);
        self::assertCount(16, $result);
        self::assertEquals($groups, $result);

        $result = $hierarchyContainer->getHierarchyLevel(Category::class);
        self::assertCount(0, $result);
        self::assertEquals([], $result);

        // Test id filter
        $result = $hierarchyContainer->whereId(2)->get();
    ...
    public function testInvalidLevelClass() : void {
        ...
        $hierarchyContainer->getHierarchyLevel('NonexistentClass');
    }
    ...
    public function testInvalidInsert() : void {
        ...
        $hierarchyContainer->insert($round);

        self::assertEquals(Round::class, $hierarchyContainer->getLevelType());
        $this->expectException(InvalidArgumentException::class);
        $hierarchyContainer->insert($category);
    }
}
