...
trait HasScore
{
    /** @var int Sum of all points acquired through the whole tournament */
    protected int $sumPoints = 0;

    /** @var int Sum of all score acquired through the whole tournament */
    ...
    public function sumPoints(array $groupIds = []) : int {
        ...
        foreach ($groupIds as $groupId) {
            $sum += $this->groupResults[$groupId]['points'] ?? 0;
        }
    ...
    public function sumScore(array $groupIds = []) : int {
        ...
        $sum = 0;
        foreach ($groupIds as $groupId) {
            $sum += $this->groupResults[$groupId]['score'] ?? 0;
        }
