...
class ImportValidator
{
     ...
     * @throws InvalidImportDataException
     */
    public static function validateParams(mixed &$data, array $keys, array $setting) : void {
        $primitive = false;
        // Check type
            ...
            switch ($setting['type']) {
                case 'array':
                    if (!is_array($data)) {
                        throw new InvalidImportDataException(
                            'Invalid data type for: ' . implode('->', $keys) . '. Expected array.'
                        );
                    }
                    // Validate subtypes
                ...
                case 'object':
                    if (!self::isObject($data)) {
                        throw new InvalidImportDataException(
                            'Invalid data type for: ' . implode('->', $keys) . '. Expected object.'
                        );
                    }
        ...
        if (isset($setting['values']) && !in_array($data, $setting['values'], true)) {
            throw new InvalidImportDataException(
                'Invalid value for: ' . implode('->', $keys) . '. Expected values: ' . implode(', ', $setting['values']) . '.'
            );
        }
        if (isset($setting['reference'])) {
     ...
     * @throws InvalidImportDataException
     */
    public static function validateType(mixed $var, array $keys, string ...$types) : void {
        ...
        throw new InvalidImportDataException(
            'Invalid data type for: ' . implode('->', $keys) . '. Expected ' . implode('|', $types) . '.'
        );
    }
    ...
    public static function isObject(mixed $data) : bool {
        return is_object($data) || (is_array($data) && [] !== $data && array_keys($data) !== range(
            0,
            count($data) - 1
        ));
    }
     ...
     * @throws InvalidImportDataException
     */
    public static function validateReference(int|string $id, string $key) : void {
        if (isset(self::$data[$key])) {
            $ids = array_map(static fn ($object) => ((object) $object)->id, self::$data[$key]);
