...
class GameSorter implements BaseSorter
{
    ...
    protected function orderCheckTeamsVal(Game $game, array $teams, array $checkVals, array $required = []) : bool {
        ...
        $requiredTeams = array_filter($teams, static fn ($a) : bool => in_array($a, $required, true));
    ...
    protected function resetGameIds() : void {
        $gameContainer = $this->group->getGameContainer();
        foreach ($this->games as $game) {
            $game->setId($gameContainer->getAutoIncrement());
            // @noinspection DisconnectedForeachInstructionInspection
            $gameContainer->incrementId();
        }
    }
}
