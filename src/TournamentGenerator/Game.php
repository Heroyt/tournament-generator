...
class Game implements WithId, Exportable, JsonSerializable
{
     ...
     */
    public function __construct(array $teams, protected Group $group) {
        $this->addTeam(...$teams);
    }
    ...
    public function getTeamsIds() : array {
        return array_map(static fn ($a) : int|string => $a->getId(), $this->teams);
    }
     ...
     *
     * @param array<int|string, int> $results array of [teamId => teamScore] key-value pairs
     *
     * @throws Exception
     */
    public function setResults(array $results = []) : Game {
        ...
        foreach ($results as $id => $score) {
            if (!is_numeric($score)) {
                throw new TypeError(
                    'Score passed to TournamentGenerator\Game::setResults() must be of the type numeric, ' . gettype(
                        $score
                    ) . ' given'
                );
            }
    ...
    public function getTeam(int|string $id) : ?Team {
        $key = array_search($id, array_map(static fn ($a) : int|string => $a->getId(), $this->teams), true);

        return false !== $key ? $this->teams[$key] : null;
    }
     ...
     * @since 0.5
     */
    public function setId(int|string $id) : Game {
        if (!is_int($id)) {
