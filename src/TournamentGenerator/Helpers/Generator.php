...
class Generator implements WithGeneratorSetters, WithSkipSetters
{
    ...
    public function genGames() : array {
        match ($this->type) {
            Constants::ROUND_ROBIN => $this->group->addGame(...$this->r_rGames()),
            Constants::ROUND_TWO   => $this->two_twoGames(),
            Constants::ROUND_SPLIT => $this->cond_splitGames(),
            default                => $this->group->getGames(),
        };
    ...
    public static function circle_saveBracket(array $teams, Group $group) : array {
        ...
        for ($i = 0; $i < count($teams) / 2; ++$i) { // GO THROUGH HALF OF THE TEAMS
            ...
            if (Constants::DUMMY_TEAM === $home || Constants::DUMMY_TEAM === $away) {
                continue;
     ...
     * @throws Exception
     */
    protected function r_r3Games(array $teams, array &$games, ?Team $team = null) : array {
        $teamsB = $teams;
        $generatedGames = [];
        while (count($teamsB) >= 3) {
            $lockedTeam = array_shift($teamsB);
            $gamesTemp = self::circle_genGames2($this->group, $teamsB);
            foreach ($gamesTemp as $gameTemp) {
                if (isset($team)) {
                    $gameTemp->addTeam($team);
                }
                $gameTemp->addTeam($lockedTeam);
            }
        ...
        for ($i = 1; $i < $iterations; ++$i) {
            ...
            foreach ($originalGames as $originalGame) {
                $games[] = $newGame = new Game(
                    ...
                    0 === $i % 2 ? $originalGame->getTeams() : array_reverse($originalGame->getTeams()),
                    $this->group
                );
                $newGames[] = $newGame;
            }
