...
class Filter
{
     ...
     * Filter constructor.
     *
     * @param TeamFilter[]|TeamFilter[][] $filters filter classes - can be structured in an hierarchical array with
     *                                             "and" / "or" keys
     * @param Group[]                     $groups  groups to consider when getting team stats
     */
    public function __construct(private readonly array $filters, array $groups) {
        $this->groups = array_filter($groups, static fn ($a) : bool => $a instanceof Group);
    }
    ...
    public function filter(array &$teams) : array {
        foreach ($this->filters as $key => $filter) {
            ...
            if ($filter instanceof TeamFilter) {
                $teams = array_filter(
                    $teams,
                    fn ($team) : bool => $filter->validate($team, $this->getGroupsIds(), 'sum', $this->groups[0])
                );
    ...
    private function getGroupsIds() : array {
        return array_map(static fn ($a) : int|string => $a->getId(), $this->groups);
    }
}
