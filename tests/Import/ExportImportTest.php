...
class ExportImportTest extends TestCase
{
     ...
     * @throws Exception
     */
    private static function tournamentSetup(bool $withSecondRound = false, bool $withScores = false) : array {
        ...
        $expectedGames[1]->scores = $group1->game([$teams[2], $teams[3]])->setResults([2 => 0o00, 3 => 500])->getResults();
        ...
        $expectedGames[5]->scores = $group1->game([$teams[1], $teams[2]])->setResults([1 => 0o00, 2 => 100])->getResults();
        ...
        $expectedGames[7]->scores = $group2->game([$teams[6], $teams[7]])->setResults([6 => 0o00, 7 => 500])->getResults();
        $expectedGames[8]->scores = $group2->game([$teams[4], $teams[6]])->setResults([4 => 100, 6 => 100])->getResults();
        $expectedGames[9]->scores = $group2->game([$teams[5], $teams[7]])->setResults([5 => 800, 7 => 0o00])->getResults();
        ...
        if ($withSecondRound) {
            ...
            foreach (array_keys($expectedSetup['progressions']) as $key) {
            ...
            $expectedGames[13]->scores = $group3->game([$teams[5], $teams[3]])->setResults([5 => 0o00, 3 => 500])->getResults();
            ...
            $expectedGames[17]->scores = $group3->game([$teams[4], $teams[5]])->setResults([4 => 0o00, 5 => 100])->getResults();
            ...
            $expectedGames[19]->scores = $group4->game([$teams[6], $teams[7]])->setResults([6 => 0o00, 7 => 500])->getResults();
            $expectedGames[20]->scores = $group4->game([$teams[1], $teams[6]])->setResults([1 => 100, 6 => 100])->getResults();
            $expectedGames[21]->scores = $group4->game([$teams[2], $teams[7]])->setResults([2 => 800, 7 => 0o00])->getResults();
    ...
    public function testExportImport(Tournament $tournament, array $expectedTeams, array $expectedGames, array $expectedSetup) : void {
        ...
        self::assertInstanceOf($tournament::class, $imported);
        ...
        foreach ($tournament->getGroups() as $group) {
            ...
            foreach ($group->getProgressions() as $key => $progression) {
                ...
                self::assertEquals(array_map(static fn (TeamFilter $teamFilter) : string => (string) $teamFilter, $progression->getFilters()), array_map(static fn (TeamFilter $teamFilter) : string => (string) $teamFilter, $importedProgressions[$key]->getFilters()));
            }
        }
    ...
    public static function getBasicTournamentData() : array {
        return [
            self::tournamentSetup(),
            self::tournamentSetup(true),
        ];
    }
}
